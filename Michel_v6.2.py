...
def schedule_phase():
    ...
    for user in users:
        ...
        user_input = "Apprend moi quelque chose s'il te plait" # Change this to the prompt you want to send to OpenAI
        try:
            completions = openai.ChatCompletion.create(
                model="gpt-3.5-turbo",
                # Append the previous responses to the prompt
...
schedule.every().day.at("19:53", "Europe/Amsterdam").do(schedule_phase)

# Define the function to run the bot and the scheduled task
async def run_bot():
    while True:
        messages = schedule_phase()
...
if __name__ == "__main__":
    ...
    asyncio.run(run_bot())
    main()  # This will run indefinitely until the bot is stopped