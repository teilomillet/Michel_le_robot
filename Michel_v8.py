...
async def handle_message(update, context):
    ...
    if len(records) == 0:
        ...
        table.create({'user_id': user_id, 'username': username, 'first_name': first_name, 'last_name': last_name, 'user_type': 'basic'})
    else:
        record = records[0]
        if 'message_count' in record['fields']:
            message_count = record['fields']['message_count'] + 1
        else:
            message_count = 1
        table.update(record['id'], {'message_count': message_count})
        
        # Check if user is a 'pro' and if their message count is over 600
        if record.get('fields', {}).get('user_type') == 'pro' and message_count >= 600:
            await message.reply_text("You have sent over 600 messages this month. Please consider upgrading your subscription plan.")
        elif record.get('fields', {}).get('user_type') == 'perso' and message_count >= 300:
            await message.reply_text("You have sent over 300 messages this month. Please consider upgrading your subscription plan.")
        elif record.get('fields', {}).get('user_type') == 'basic' and message_count >= 31:
            if message_count == 31:
                await message.reply_text("You have sent 31 messages this month. Please consider upgrading your subscription plan.")
            else:
